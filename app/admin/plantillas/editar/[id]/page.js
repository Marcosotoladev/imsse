// app/admin/plantillas/editar/[id]/page.js - Editar Plantilla de Visita Técnica (admin)
'use client';

import { useState, useEffect, use } from 'react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { Home, Save, Trash2, PlusCircle, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function EditarPlantilla({ params }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiService.obtenerPlantillaInspeccionPorId(id);
        setPlantilla({
          grupo: data.grupo || GRUPOS_INSPECCION[0],
          titulo: data.titulo || '',
  ...
  const handleGuardar = async () => {
    ...
    setGuardando(true);
    try {
      await apiService.actualizarPlantillaInspeccion(id, payload);
      router.push('/admin/plantillas');
    } catch (error) {
        ...
        <div className="flex flex-wrap items-center justify-between mb-8">
          ...
          <div className="flex mb-4 space-x-2">
            <button
              ...
              disabled={eliminando}
              className="flex items-center px-4 py-2 text-red-600 transition-colors bg-red-50 rounded-md hover:bg-red-100 disabled:opacity-50"
            >
