// app/admin/plan-accion/nueva/page.js - Nueva propuesta del Plan de Acción (admin)
'use client';
...
export default function NuevaPropuestaPlanAccion() {
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      await apiService.crearPlanAccion(datos);
      router.push('/admin/plan-accion');
    } catch (error) {
      console.error('Error al guardar la propuesta:', error);
      alert('❌ Error al guardar la propuesta. Inténtelo de nuevo.');
    } finally {
      setGuardando(false);
    }
  };
