...
export async function verifyAuth(req) {
  try {
    ...
    return {
      ...
      superAdmin: userData.superAdmin === true // Acceso invisible, siempre bypassea el modo mantenimiento
    };
  } catch (error) {
...
export function withAuth(handler, requiredPermissions = [], options = {}) {
  const { allowDuringMaintenance = false } = options;
  ...
  return async (req, res) => {
    try {
      ...
      // Modo mantenimiento: si la suscripción está vencida, se bloquean las acciones
      // (todo lo que no sea GET) para cualquiera que no sea superAdmin.
      // allowDuringMaintenance es la salida para el propio endpoint de pago: el admin
      // bloqueado tiene que poder generar su link de MercadoPago para destrabarse solo.
      if (req.method !== 'GET' && !user.superAdmin && !allowDuringMaintenance) {
        const { bloqueada } = await getSubscriptionState();
        if (bloqueada) {
          return res.status(503).json({
            error: 'La aplicación está en modo mantenimiento',
            maintenance: true
          });
        }
      }
