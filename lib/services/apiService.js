...
class ApiService {
  ...
  // Método genérico para hacer requests. Con timeout: con señal mala un fetch puede
  // quedar colgado sin fallar nunca, dejando el spinner infinito en vez de caer al
  // fallback offline.
  async makeRequest(url, options = {}) {
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        ...
        throw new Error('Tiempo de espera agotado. Verificá tu conexión.');
      }
  ...
  async cambiarPasswordUsuario(targetUid, newPassword) {
  ...
  async migrarEmpresasHistoricas() {
    return await this.makeRequest('/api/admin/backfill-empresas', {
      method: 'POST'
    });
  }
  ...
  async obtenerInspeccionesTecnicas(filtros = {}) {
    return this.obtenerDocumentos('inspecciones', filtros);
  }

  async crearInspeccionTecnica(data) {
    return this.crearDocumento('inspecciones', data);
  }

  async obtenerInspeccionTecnicaPorId(id) {
    return this.obtenerDocumento('inspecciones', id);
  }

  async actualizarInspeccionTecnica(id, data) {
    return this.actualizarDocumento('inspecciones', id, data);
  }

  async eliminarInspeccionTecnica(id) {
    return this.eliminarDocumento('inspecciones', id);
  }

  // Plantillas de Inspección
  async obtenerPlantillasInspeccion(filtros = {}) {
    return this.obtenerDocumentos('plantillas', filtros);
  }

  async crearPlantillaInspeccion(data) {
    return this.crearDocumento('plantillas', data);
  }

  async obtenerPlantillaInspeccionPorId(id) {
    return this.obtenerDocumento('plantillas', id);
  }

  async actualizarPlantillaInspeccion(id, data) {
    return this.actualizarDocumento('plantillas', id, data);
  }

  async eliminarPlantillaInspeccion(id) {
    return this.eliminarDocumento('plantillas', id);
  }

  // Plan de Acción
  async obtenerPlanAccion(filtros = {}) {
    return this.obtenerDocumentos('planaccion', filtros);
  }

  async crearPlanAccion(data) {
    return this.crearDocumento('planaccion', data);
  }

  async obtenerPlanAccionPorId(id) {
    return this.obtenerDocumento('planaccion', id);
  }

  async actualizarPlanAccion(id, data) {
    return this.actualizarDocumento('planaccion', id, data);
  }

  async eliminarPlanAccion(id) {
    return this.eliminarDocumento('planaccion', id);
  }
  ...
  async obtenerNotificaciones(filtros = {}) {
    ...
    return await this.makeRequest(`/api/notificaciones${queryString}`);
  }

  async crearNotificacion(mensaje) {
    return await this.makeRequest('/api/notificaciones', {
      method: 'POST',
  ...
  async actualizarNotificacion(id, data) {
    return await this.makeRequest(`/api/notificaciones/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
  }
  ...
  async marcarNotificacionesLeidas() {
    return await this.makeRequest('/api/notificaciones/marcar-leidas', {
      method: 'POST'
    });
  }

  async obtenerNotificacionesNoLeidas() {
    return await this.makeRequest('/api/notificaciones/no-leidas');
  }

  // ========== SUSCRIPCIÓN ==========

  async obtenerSuscripcion() {
    return await this.makeRequest('/api/sistema/suscripcion');
  }

  async actualizarSuscripcion(data) {
    return await this.makeRequest('/api/sistema/suscripcion', {
      method: 'PUT',
      body: JSON.stringify(data)
    });
  ...
  async generarCobroSuscripcion(data) {
    return await this.makeRequest('/api/sistema/suscripcion/generar-cobro', {
      method: 'POST',
      body: JSON.stringify(data)
    });
  }
  ...
  async cancelarSuscripcion() {
    return await this.makeRequest('/api/sistema/suscripcion/cancelar', {
      method: 'POST'
    });
  }
