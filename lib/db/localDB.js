...
class LocalDB {
  ...
  async init() {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Si una pestaña vieja quedó con la conexión abierta, el upgrade queda bloqueado
      // y `request.onsuccess` nunca dispara — sin esto, cualquier llamador queda
      // colgado esperando una promesa que no resuelve ni rechaza nunca.
      request.onblocked = () => {
        console.warn('IndexedDB bloqueada por otra pestaña: cerrá otras pestañas de la app y volvé a intentar.');
      ...
      request.onupgradeneeded = (event) => {
        ...
        // Store para inspecciones técnicas
        if (!db.objectStoreNames.contains('inspecciones')) {
          const inspeccionesStore = db.createObjectStore('inspecciones', {
            keyPath: 'localId',
            autoIncrement: true
          });
          inspeccionesStore.createIndex('syncStatus', 'syncStatus', { unique: false });
          inspeccionesStore.createIndex('serverId', 'serverId', { unique: false });
          console.log('Store inspecciones creado');
        }

        // Store para borradores: autoguardado local mientras se completa un formulario
        // de OT/VT, sin relación con la cola de sincronización (nunca se sube al
        // servidor, es puramente una red de seguridad contra cerrar la app sin guardar)
        if (!db.objectStoreNames.contains('borradores')) {
          db.createObjectStore('borradores', { keyPath: 'clave' });
          console.log('Store borradores creado');
        }
  ...
  async getOrdenes(tecnicoId) {
    ...
    return new Promise((resolve, reject) => {
      // Sin tecnicoId, `index.getAll(undefined)` trae los registros de TODOS los
      // técnicos en vez de nada útil — se usa el store completo explícitamente.
      const request = tecnicoId
        ? store.index('tecnicoId').getAll(tecnicoId)
        : store.getAll();
      request.onsuccess = () => {
  ...
  async updateOrden(localId, updates) {
    await this.init();
    ...
    return new Promise((resolve, reject) => {
      const getRequest = store.get(localId);
      getRequest.onsuccess = () => {
        const orden = getRequest.result;
        if (!orden) {
          reject(new Error('Orden no encontrada'));
          return;
        }

        const ordenActualizada = { ...orden, ...updates };
        const putRequest = store.put(ordenActualizada);
        putRequest.onsuccess = () => resolve(putRequest.result);
        putRequest.onerror = () => reject(putRequest.error);
      };
      getRequest.onerror = () => reject(getRequest.error);
  ...
  async findOrdenByServerId(serverId) {
    await this.init();
  ...
  async saveInspeccion(inspeccion) {
    ...
    const transaction = this.db.transaction(['inspecciones'], 'readwrite');
    const store = transaction.objectStore('inspecciones');

    const inspeccionLocal = {
      ...inspeccion,
      syncStatus: inspeccion.syncStatus || 'pending',
      localTimestamp: new Date().toISOString(),
      serverId: inspeccion.id || inspeccion.serverId || null
    };

    return new Promise((resolve, reject) => {
      const request = store.add(inspeccionLocal);
      request.onsuccess = () => resolve(request.result);
      request.onerror = () => reject(request.error);
    });
  }
  ...
  async getInspecciones() {
    ...
    const transaction = this.db.transaction(['inspecciones'], 'readonly');
    const store = transaction.objectStore('inspecciones');

    return new Promise((resolve, reject) => {
      const request = store.getAll();
      request.onsuccess = () => {
        const inspecciones = request.result.filter((i) => !i.deleted);
  ...
  async updateInspeccion(localId, updates) {
    await this.init();
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        if (!inspeccion) {
          reject(new Error('Inspección no encontrada'));
          return;
        }

        const inspeccionActualizada = { ...inspeccion, ...updates };
        const putRequest = store.put(inspeccionActualizada);
        putRequest.onsuccess = () => resolve(putRequest.result);
        putRequest.onerror = () => reject(putRequest.error);
      };
      getRequest.onerror = () => reject(getRequest.error);
  ...
  async deleteInspeccion(localId) {
    await this.init();
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        inspeccion.deleted = true;
        inspeccion.deletedAt = new Date().toISOString();

        const putRequest = store.put(inspeccion);
        putRequest.onsuccess = () => resolve(putRequest.result);
        putRequest.onerror = () => reject(putRequest.error);
      };
      getRequest.onerror = () => reject(getRequest.error);
  ...
  async findInspeccionByServerId(serverId) {
    await this.init();
  ...
  async findRecordatorioByServerId(serverId) {
    await this.init();
  ...
  // ========== BORRADORES (autoguardado local, no se sincroniza al servidor) ==========

  async guardarBorrador(clave, datos) {
    await this.init();

    const transaction = this.db.transaction(['borradores'], 'readwrite');
    const store = transaction.objectStore('borradores');

    return new Promise((resolve, reject) => {
      const request = store.put({ clave, datos, fecha: new Date().toISOString() });
      request.onsuccess = () => resolve(request.result);
      request.onerror = () => reject(request.error);
    });
  }

  // Devuelve el borrador guardado, o undefined si no hay o si tiene más de 7 días
  // (se descarta solo, no tiene sentido ofrecer recuperar un trabajo tan viejo).
  async obtenerBorrador(clave) {
    await this.init();

    const transaction = this.db.transaction(['borradores'], 'readonly');
    const store = transaction.objectStore('borradores');
    ...
    const antiguedadMs = Date.now() - new Date(registro.fecha).getTime();
    if (antiguedadMs > 7 * 24 * 60 * 60 * 1000) {
      await this.eliminarBorrador(clave);
      return undefined;
    }
  ...
  async clearAllData() {
    ...
    const stores = ['marcaciones', 'ordenes', 'recordatorios', 'inspecciones', 'borradores', 'syncQueue'];
  ...
  async getPendingSyncItems() {
    ...
    return syncQueue.filter(item => item.retries < 3 && item.estado !== 'fallido'); // Solo intentar 3 veces
  }
}
